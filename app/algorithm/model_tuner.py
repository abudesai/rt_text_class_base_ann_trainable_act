...
def tune_hyperparameters(
    ...
    def objective(**hyperparameters):
        ...
        train_X, train_y = train_data["X"].astype(np.float16), train_data["y"].astype(
            np.float16
        )
        valid_X, valid_y = valid_data["X"].astype(np.float16), valid_data["y"].astype(
            np.float16
        )
        ...
        score = model.evaluate(valid_X, valid_y)[
            0
        ]  # returns cross-entropy and accuracy, so indexing at 0
        ...
        opt_metric = np.round(
            score + np.random.normal(0, 1e-4), 5
        )  # returns binary_cross_entropy
